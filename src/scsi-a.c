...
static void process_SelectionPhase()
{
        ...
        target && likely(maskBitCount <= 2)) // RM make it simple
	{
		...
		SCSI_CTL_BSY_Write(1); // RM Register instead pin
		ledOn();
		...
		if (!scsiDev.atnFlag)
		{
			...
			scsiDev.compatMode = COMPAT_SCSI1; // RM ACSI should be here
		}
		...
		// section.
		// SCSI1/SASI initiators may not set their own ID. RM ACSI won't
		// {
		//	int i;
		...
		while (likely(!scsiDev.resetFlag))
		{
            ...
            //        		scsiDev.target->cfg->quirks == CONFIG_QUIRKS_XEBEC); // RM
            //		{
...
static void process_MessageOut()
{
	...
	scsiDev.atnFlag = 0; // |= SCSI_ReadFilt(SCSI_Filt_ATN); RM
}
...
int scsiReconnect()
{
	...
	if (!sel && !bsy)
	{
		...
		{
			// Won arbitration
			// SCSI_SetPin(SCSI_Out_SEL); // RM ACSI won't arbitrate
			...
			if (bsy)
			{
				...
				scsiDeskewDelay(); // 2 deskew delays
				scsiDeskewDelay(); // 2 deskew delays
				// SCSI_ClearPin(SCSI_Out_SEL); // RM ACSI won't arbitrate
